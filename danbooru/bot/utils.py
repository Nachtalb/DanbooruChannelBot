    ...
    def runtime_settings_command(self, bot: Bot, update: Update):
        ...
        if old_value_type in (list, set, tuple):
            ...
            update.message.reply_text(
                f"Setting `{setting}` was changed: {action_name} `{value}` in `{old_value}`",
            ...
            if old_value_type in (int, float):
                try:
                    ...
                    update.message.reply_text(
                        f"Value must be `{old_value_type.__name__}`", parse_mode=ParseMode.MARKDOWN
                    )
                    return
            setattr(settings, setting, value)
            update.message.reply_text(
                f"Setting `{setting}` was changed from `{old_value}` to `{value}`", parse_mode=ParseMode.MARKDOWN
            )
