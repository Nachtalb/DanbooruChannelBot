...
class Command:
    ...
    SAFE_CONFIG_KEYS = {
        'artist', 'buttons', 'characters', 'date_format', 'id', 'suffix',
        'tags', 'time', 'no_file', 'direct_button', 'force_file',
        'explicit_file', 'questionable_file', 'debug',
    }
    UNSAFE_CONFIG_KEYS = {
        'subs'
    }
    ...
    def __init__(self):
        ...
        danbooru_bot.add_command(name='config', func=self.change_config_command_wrapper, admin=False, pass_args=True, run_async=True)
        danbooru_bot.add_command(name='sub', func=self.subscribe_command, admin=False, pass_args=True, run_async=True)
        danbooru_bot.add_command(name='unsub', func=self.unsubscribe_command, admin=False, pass_args=True, run_async=True)
        danbooru_bot.add_command(name='gsub', func=self.subscribe_group_command, admin=False, pass_args=True, run_async=True)
        danbooru_bot.add_command(name='gunsub', func=self.unsubscribe_group_command, admin=False, pass_args=True, run_async=True)
    ...
    def create_post(self,
                    ...
                    config: dict={},
                    no_file: bool=False,
                    force_file: bool=False) -> tuple[Callable, Dict] | None:
        tags = set(post.tag_string.split(' '))
        caption = ''
        ...
        if config.get('time', settings.SHOW_DATE):
            caption += '\n<b>Posted at:</b> %s' % post.created_at.strftime(config.get('date_format', settings.DATE_FORMAT))
    ...
    def start_scheduler(self):
        ...
        self.job = danbooru_bot.updater.job_queue.run_repeating(
            self.refresh, interval=timedelta(minutes=settings.RELOAD_INTEVAL),
            first=1, name='danbooru_refresh')
